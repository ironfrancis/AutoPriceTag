    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 px-3 py-2 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-semibold text-gray-900">选择标签</h3>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isLoading ? (
          ...
          <div className="text-center py-6">
            <p className="text-xs text-gray-500">暂无保存的标签</p>
            <p className="text-xs text-gray-400 mt-1">请先到标签编辑器创建并保存标签</p>
            <div className="mt-3">
              <a 
                href="/editor"
          ...
          <div className="space-y-1.5">
            {savedLabels.map((label, index) => {
              ...
              return (
                <div
                  key={labelId}
                  className="p-2 border border-gray-200 rounded-md cursor-pointer transition-colors hover:border-blue-500 hover:bg-blue-50"
                  onClick={() => handleAddLabel(label)}
                >
                  <div className="flex items-start gap-2">
                    {/* 添加图标 */}
                    <div className="flex items-center justify-center w-6 h-6 border-2 border-blue-500 bg-blue-50 rounded flex-shrink-0">
